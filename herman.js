//fahril bot
//DONASI YA BANG UDAH SUSAH2 UPDATE
//BANTU YUK SEWA/DONASI BIAR BOT BISA STAY 24 JAM
//AYO BANTU LAH BANG DONASI
//>>SEWA BOT HUB OWNER<<*//
const {
    WAConnection,
	MessageType,
...
let errorImg = 'https://i.ibb.co/nLtSF2V/20211222-071710.jpg'
...
var hayuk0 = '[NOT VERIFIED]'
      ...
      const demoteAdmin = async function(to, target=[]){
          ...
          if (!packname) packname = 'Herman'; if (!author) author = 'Chanel';author = author.replace(/[^a-zA-Z0-9]/g, '');	
...
const isImage = (type === 'imageMessage')
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
      ...
      const daftar1 = `Hai kak  ${pushname} ${ucapanWaktu} \n\nSebelum Menggunakan Bot Verify Terlebih Dahulu Ya `
       const daftar2 = '```Ketik Tombol Di Bawah Untuk Daftar Ya Kak\nUntuk Wa Mod Silahkan .verify```'
       const daftar3 = [
          {
            buttonId: `!verify`,
            buttonText: {
      ...
      // GAME 
        ...
        if (game.isfam(from, family100)) {
          ...
          if (tebakata.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
			...
			/////
let ikyads = await getBuffer(`https://i.ibb.co/nLtSF2V/20211222-071710.jpg`)
let herman1 = await getBuffer(`https://i.ibb.co/gt0SK9f/20211222-132009.jpg`)
let herman2 = await getBuffer(`https://i.ibb.co/tPqw98S/20211222-132025.jpg`)
let herman3 = await getBuffer(`https://i.ibb.co/SRryVcs/20211222-132038.jpg`)
fakelink = (tekslink) => { 
	return {"externalAdReply": { "title": tekslink, "thumbnail": ikyads, "sourceUrl": "https://wa.me/qr/ZJOIL22B4D7YJ1" }}
}
// AFK
	if (isGroup) {
...
*NOMOR CREATOR*
▼▼▼▼▼`)
sendKontak(from, `6285876902820`, `${ownerName}`, 'Sibukk!!')
               await sleep(1000)
               break
               case 'spamcall':{
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: freply})
       ...
       menu =`${ucapanWaktu} 🤭
 ❉ *BOT INFO* ❉
⬡ ⃝⃕ׄ🐲 *BOT NAME* : ${botName}
⬡ ⃝⃕ׄ🐲 *OWNER* : ${ownerName}
⬡ ⃝⃕ׄ🐲 *SPEED* : *${latensie.toFixed(4)} _Second_*
⬡ ⃝⃕ׄ🐲 *ACTIV* : *${runtime(process.uptime())}*
⬡ ⃝⃕ׄ🐲 *PLAYFORM* : *${os.platform()}*
...
「 ${botName} 」`
			...
			reply(`jangan lupa donasi biar bot bisa stay 24 jam 🗿`)
		}, 2000)
                    ikyy.sendMessage(from, { contentText: `${menu}`, footerText: '⚠️ Jika Tombol Button Tidak Keliatan Langsung Ketik !allmenu Dilarang Keras Menyepam Bot!!\n𝘧𝘢𝘩𝘳𝘪𝘭 𝘣𝘰𝘵𝘻☕', buttons: [{ buttonId: `!diamond`, buttonText: { displayText: 'DIAMOND💎' }, type: 1 },{ buttonId: `!allmenu`, buttonText: { displayText: 'ALLMENU🍂' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: 'OWNER 😍' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ikyads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break
case 'f':
case 'lock':
 ikymemek = {
  "key": {
    "fromMe": false,
    participant: `6285876902820@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
  },
  "message": {
    "viewOnceMessage": {
      "message": {
...
Don't give up, remember that in the sky there is still a sky. We eat together rice, don't be afraid
...
✗  ${prefix}ytsearch *query*`
			ikyy.updatePresence(from, Presence.composing)
                    ikyy.sendMessage(from, { contentText: `${menu}`, footerText: 'Peringatan !!! ⚠️Jangan Spam Bot yaa!! Kamu Gatau Betapa Susah Nya Bikin Botz:v\nℎ𝑒𝑟𝑚𝑎𝑛 𝐵𝑜𝑡𝑧☕', buttons: [{ buttonId: `!menu`, buttonText: { displayText: 'BACK TO MENU🔙' }, type: 1 },{ buttonId: `!donasi2`, buttonText: { displayText: 'DONASI😄' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ikyads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break
...
SEBELUM MELANJUTKAN PASTIKAN ANDA BERUMUR 18+ dan menu ini juga tidak di cantumkan di allmenu karena bersivat negative
`
              ...
              ikyy.relayWAMessage(prep)
                ...
                reply (mess.wait)
                  ...
                  buff = await getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${setting.lolkey}&text=${ini_txt}`)
                          buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'ℎ𝑒𝑟𝑚𝑎?? ??𝑜𝑡𝑧☕T', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @cal_me_herman`,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
...
`
 ...
 ℎ𝑒𝑟𝑚𝑎𝑛 𝐵𝑜𝑡𝑧☕︎`, imageMessage: imageMsg,
               ...
               prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
                          ikyy.relayWAMessage(prep)
               break
case 'diamondml': //case by Herman Chanel Hargai Gue cuk
       thu = await ikyy.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: freply})
       menu =`List Harga Diamond Ml
3 diamonds💎	1.500.
12 diamonds💎  3.325.
28 diamonds💎  7.600.
36 diamonds💎  9.500.
...
6050 diamonds💎	1.425.000.`
			ikyy.updatePresence(from, Presence.composing)
                    ikyy.sendMessage(from, { contentText: `${menu}`, footerText: 'Jangan Lupa sewa/donasi bot biar bot bisa stay 24 jam 🗿', buttons: [{ buttonId: `!ya`, buttonText: { displayText: 'BAYAR 🎟️'}, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: 'OWNER👤' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: herman1, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
  
      break
case 'diamondff': //case by Herman Chanel Hargai Gue cuk
       thu = await ikyy.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: freply})
       menu =`List Harga Diamond Ff
5 diamonds💎	1.000.
12 diamonds💎	 2.000.
50 diamonds💎	 8.000.
70 diamonds 💎	 10.000.
...
73100 diamonds💎	10.000.000`
			ikyy.updatePresence(from, Presence.composing)
                    ikyy.sendMessage(from, { contentText: `${menu}`, footerText: 'Jangan Lupa Subscribe Herman Chanel Terus Nyalain Lonceng Nya Biar Ga ketinggalan Video Terbaru Nya\nℎ𝑒𝑟𝑚𝑎𝑛 𝐵𝑜𝑡𝑧☕', buttons: [{ buttonId: `!ya`, buttonText: { displayText: 'BAYAR 🎟️'}, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: 'OWNER👤' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: herman2, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
  
      break
case 'ucpubg': //case by Herman Chanel Hargai Gue cuk
       thu = await ikyy.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: freply})
       menu =`List Harga UC Pubg
💵50	Rp 20.000
💵125	Rp 35.000
💵250	Rp 60.000
💵500	Rp 115.000
...
`
			ikyy.updatePresence(from, Presence.composing)
                    ikyy.sendMessage(from, { contentText: `${menu}`, footerText: 'Jangan Lupa Subscribe Herman Chanel Terus Nyalain Lonceng Nya Biar Ga ketinggalan Video Terbaru Nya\nℎ𝑒𝑟𝑚𝑎𝑛 𝐵𝑜𝑡𝑧☕', buttons: [{ buttonId: `!ya`, buttonText: { displayText: 'BAYAR 🎟️'}, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: 'OWNER👤' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: herman3, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: freply})
       menu =`Halo ${pushname} 🤗`
			ikyy.updatePresence(from, Presence.composing)
                    ikyy.sendMessage(from, { contentText: `${menu}`, footerText: 'Donasi Ya Buat Beli Kouta Biar Bot Nya Online Terus', buttons: [{ buttonId: `!kntl`, buttonText: { displayText: 'NANTI AJA BANG🗿'}, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ikyads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
...
`
 ...
 ℎ𝑒𝑟𝑚𝑎𝑛 𝐵𝑜𝑡𝑧☕︎`, imageMessage: imageMsg,
   ...
   case 'diamond':
               list = []
               listmenu = [`diamondml`,`owner`,`diamondff`,`ucpubg`]
               listmenuu = [`diamondml`,`owner`,`diamondff`,`ucpubg`]
               nombor = 1
               startnum = 0
               ...
               listmsg(from, `silahkan pilih`, `  `, list)
               break
   case 'izinout':
   list = []
               listmenu = [`adagc`,`menu`]
...
*「 ℎ𝑒𝑟𝑚𝑎𝑛 𝐵𝑜𝑡𝑧☕︎ 」*`
	    ...
	    // console.log(e)
            setTimeout( () => {
			ikyy.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Mendaftar Di fahril bot🐟*`)
		}, 2000)
        break
case 'sfire':
{
...
`
  buttons = [{buttonId: `${prefix}truth`,buttonText:{displayText: `Truth`},type:1},{buttonId: `${prefix}dare`,buttonText:{displayText: `Dare`},type:1}]
      ...
      break
       case 'belipremium':
       case 'buypremium':
              gopeynya = 'https://i.ibb.co/nLtSF2V/20211222-071710.jpg'
            buff = await getBuffer(gopeynya)
            teksnya = `
┏━━⬣ PREMIUM!
┃⬡ 1 MINGGU 10K
...
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `OWNER`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'fahril botz☕︎', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
  break
...
//------------------< Owner >-------------------
      ...
      case 'broadcast':
             ...
             for (let _ of anu) {
             sendMess(_.jid, `*「fahril broadcast」*\n\n${body.slice(4)}`)
}
             reply('Suksess broadcast')
}
             break
...
ikyy.sendMessage(_.jid, 
			{"contentText": `*「 Fahril Broadcast 」*\n\n${body.slice(4)}`,
			"footerText": '𝘧𝘢𝘩𝘳𝘪𝘭 𝘣𝘰𝘵𝘻 ㌹',
			"buttons": [
			{"buttonId": `#command`,
...
//------------------< Fun >-------------------
       case 'wangy':
              if (!q) return
              qq = q.toUpperCase()
              awikwok = `${qq} ${qq} ${qq} ❤️ ❤️ ❤️ WANGY WANGY WANGY WANGY, aaaq} wangyy aku mau nyiumin aroma wangynya ${qq} AAAAAAAAH ~ Rambutnya.... aaah rambutnya juga pengen aku elus-elus ~~ AAAAAH ${qq} keluar pertama kali di anime juga manis ❤️ ❤️ ❤️ banget AAAAAAAAH ${qq} AAAAA LUCCUUUUUUUUUUUUUUU............ ${qq} AAAAAAAAAAAAAAAAAAAAGH ❤️ ❤️ ❤️apa ? ${qq} itu gak nyata ? Cuma HALU katamu ? nggak, ngak ngak ngak ngak NGAAAAAAAAK GUA GAK PERCAYA ITU DIA NYATA NGAAAAAAAAAAAAAAAAAK PEDULI BANGSAAAAAT !! GUA GAK PEDULI SAMA KENYATAAN POKOKNYA GAK PEDULI. ❤️ ❤️ ❤️ ${qq} gw ... ${qq} di laptop ngeliatin gw, ${qq} .. kamu percaya sama aku ? aaaaaaaaaaah syukur ${q} aku gak mau merelakan ${qq} aaaaaah ❤️ ❤️ ❤️ YEAAAAAAAAAAAH GUA MASIH PUNYA ${qq} SENDIRI PUN NGGAK SAMA AAAAAAAAAAAAAAH`
              reply(awikwok)
              break
       case 'bismilah':
       case 'bismillah':
...
//------------------< Lainnya >-------------------
        ...
        case 'tes':
               ...
               break
case 'kntl':
               reply('Ya Elah Pelit Amat Lu Bang🗿')
               break
        case 'info':  // Jangan Di Ubah Plise
               urlinfo = 'https://i.ibb.co/nLtSF2V/20211222-071710.jpg'
